...
const VERSION="ca26-app-v4", TILES="ca26-tiles", PHOTOS="ca26-photos";
...
self.addEventListener("fetch",e=>{
  ...
  if(u.hostname==="upload.wikimedia.org"){ /* species photos: saved once, then served from the phone */
    e.respondWith(caches.open(PHOTOS).then(async c=>{const hit=await c.match(e.request.url);if(hit)return hit;
      try{const r=await fetch(e.request.url,{mode:"cors"});if(r.ok)c.put(e.request.url,r.clone());return r}catch(err){return new Response("",{status:504})}}));return}
